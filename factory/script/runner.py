...
from factory.script.parser import Types
...
# for an inline non-pausing/non-remote function, you only need to implement execute and value, toAssembler is not called if done is immeditally True.
...
# Keep in mind that toAssembler may be called without the resulting value ever making it to assembler.  Network issues, multiple assemblers
# not all containing all the needed plugins, etc may cause toAssembler's output to be discarded.  It may also be possible that assembler might
# lock up and never return it's results to fromAssembler (lock up, network issues, process terminated, etc.), design acordingly.

# it is perfectly acceptable to not complete the remote function in one request, if some communication betwee factory and assembbler modules is needed
# this is allowed, keep in-mind, you may need to handle retries.  When handeling retries, do not rely on counting the number of times toAssembler/run/done/message
# are called, it is acceptable to store local timers.

# any communication between the factory and assembler pairs must happen between to/fromAssembler calls, if an operation requires multiple
# communicatoin routnds, keep in mind that it is possible for multiple assembler to be taking requests, so assembler must store all state
# with factory
...
reset/rollback event:
  rollback()
  if values to assembler is outstanding, that is re-sent
...
following executions:
  if done is False:
    ...
    if value dispatched to assembler and returned:
      toAssembler() -> if not None, value dispatched to assembler

  if done is True:
    value is retreived and returned to script

Assembler returns result:
  fromAssembler()
  set job status to status
"""


# the module value sent to assembler is the same as the script name
...
# as the module name for assembler, and the second is the function
...
class ExternalFunction( object ):
  ...
  def done( self ):
    ...
    # this is called every time factory wants to know if this script can continue.
    # keep in mind that assembler, users, and other processes cause this to be called
  ...
  @property
  def message( self ):
    # this is called when done returns False, or after fromAssembler is called
  ...
  def rollback( self ):
    # this is call if there has been an error or loss of communication/tasks with assembler
    ...
    # this must setup the process of rolling back, the regular run, to/fromAssembler
  ...
  def toAssembler( self ):
    # this is sent to the assembler
    ...
    # this is called initially, then again after fromAssembler has returnd results until done is True
    # example: return ( 'myfunc', { 'stuff': 'for', 'myfunc': 'to use' } ) # NOTE: the paramater part can be anything that is serilizable
    # if None is returned, assembler will not be notified to do anything
    # THIS MUST NOT HANG/PAUSE/WAIT/POLL
    # assembler threads will be waiting on this function
    pass

  def fromAssembler( self, data ):
    # data will be a dict
    # this will be called once for every toAssembler task sent to assembler.  There could be external
    # logic that looks for timeout/loss of results and call rollback to start from the top
    # THIS MUST NOT HANG/PAUSE/WAIT/POLL
    # assembler threads will be waiting on this function, don't do anything to heavy to hold up factory/assembler
    # if there is a problem with the data from assembler or some other fatal error occurs,
...
class Runner( object ):
      ...
      if self.state[ state_index ][1] is None:  # TODO: is there a better way to handle this?
        ...
        try:
          ...
          if isinstance( handler, ExternalFunction ):
            ...
            self.factory_cookie = str( uuid.uuid4() )
  ...
  def toAssembler( self, assembler_module_list ):
    ...
    try:
      if operation[1][ 'module' ] not in assembler_module_list:
        return None
    except KeyError:
    ...
    handler._runner = self
    try:
      paramaters = handler.toAssembler()
    except Exception as e:
      _debugDump( 'Handler "{0}" in module "{1}" error during toAssembler on line "{2}"'.format( handler.__class__.__name__, operation[1][ 'module' ], self.cur_line ), e, self.ast, self.state )
    ...
    return { 'module': operation[1][ 'module' ], 'function': paramaters[0], 'cookie': self.factory_cookie, 'paramaters': paramaters[1] }

  def fromAssembler( self, cookie, data ):
    if self.done or self.aborted or self.state == []:
      return ( 'Script not Running', None )

    if cookie != self.factory_cookie:
    ...
    handler._runner = self
    try:
      handler.fromAssembler( data )
    except Exception as e:
      _debugDump( 'Handler "{0}" in module "{1}" error during fromAssembler on line "{2}"'.format( handler.__class__.__name__, operation[1][ 'module' ], self.cur_line ), e, self.ast, self.state )
  ...
  def rollback( self ):  # TODO: make to/from assembler and rollback consistant in how they handle errors, this will take some work with the things calling them
    if self.done or self.aborted or self.state == []:
    ...
    self.factory_cookie = str( uuid.uuid4() )  # revoke any outstanding tasks, TODO: do we also rotate cookie on reset?  if not, should we rotate keys even if rollback is  not possible
    operation[1][ 'dispatched' ] = False
  ...
  def registerModule( self, name ):
    ...
    self.function_map[ module.SCRIPT_NAME ] = module.SCRIPT_FUNCTIONS
    self.value_map[ module.SCRIPT_NAME ] = module.SCRIPT_VALUES
  ...
  def __getstate__( self ):
    return { 'module_list': self.module_list, 'object_list': self.object_list, 'state': self.state, 'variable_map': self.variable_map, 'cur_line': self.cur_line, 'factory_cookie': self.factory_cookie }

  def __setstate__( self, state ):
    self.state = state[ 'state' ]
    self.variable_map = state[ 'variable_map' ]
    self.cur_line = state[ 'cur_line' ]
    self.factory_cookie = state[ 'factory_cookie' ]
