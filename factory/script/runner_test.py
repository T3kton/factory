...
from factory.script.parser import parse
from factory.script.runner import Runner, ExecutionError, UnrecoverableError, ParamaterError, NotDefinedError, Timeout, Pause

# TODO: test the assignment deepcopy, ie: a = {}, b = a  make sure changes to b are not reflected in a


class testExternalObject( object ):
  SCRIPT_NAME = 'test_obj'
...
def test_module_values():  # TODO: add pickling testing
  ...
  runner.registerModule( 'factory.script.runner_plugins_test' )
  assert runner.variable_map == {}
  runner.run()
  assert runner.done
...
def test_external_remote_functions():
  ...
  assert runner.toAssembler( [ 'testing' ] ) is None
  assert runner.line == 0
  assert runner.fromAssembler( runner.factory_cookie, True ) == ( 'Script not Running', None )
  ...
  assert runner.fromAssembler( runner.factory_cookie, True ) == ( 'Not Expecting Anything', None )
  assert runner.toAssembler( [] ) is None
  assert runner.toAssembler( [ 'sdf', 'were' ] ) is None
  assert runner.toAssembler( [ 'rfrf', 'testing', 'sdf' ] ) == { 'cookie': runner.factory_cookie, 'module': 'testing', 'function': 'remote_func', 'paramaters': 'the count "1"' }
  ...
  assert runner.status == [ ( 0.0, 'Scope', { 'description': 'Overall Script' } ), ( 0.0, 'Function', { 'module': 'testing', 'name': 'remote', 'dispatched': True } ) ]
  assert runner.toAssembler( [ 'testing' ] ) is None
  assert runner.fromAssembler( 'Bad Cookie', True ) == ( 'Bad Cookie', None )
  assert runner.fromAssembler( runner.factory_cookie, True ) == ( 'Accepted', 'Current State "True"' )
  assert runner.status == [ ( 0.0, 'Scope', { 'description': 'Overall Script' } ), ( 0.0, 'Function', { 'module': 'testing', 'name': 'remote', 'dispatched': False } ) ]
  assert runner.fromAssembler( runner.factory_cookie, True ) == ( 'Not Expecting Anything', None )
  assert runner.toAssembler( [ 'testing' ] ) == { 'cookie': runner.factory_cookie, 'module': 'testing', 'function': 'remote_func', 'paramaters': 'the count "2"' }
  assert runner.status == [ ( 0.0, 'Scope', { 'description': 'Overall Script' } ), ( 0.0, 'Function', { 'module': 'testing', 'name': 'remote', 'dispatched': True } ) ]
  assert runner.fromAssembler( runner.factory_cookie, True ) == ( 'Accepted', 'Current State "True"' )
  assert runner.status == [ ( 0.0, 'Scope', { 'description': 'Overall Script' } ), ( 0.0, 'Function', { 'module': 'testing', 'name': 'remote', 'dispatched': False } ) ]
  assert runner.run() == ''
  ...
  assert not runner.done
  assert runner.status == [ ( 0.0, 'Scope', { 'description': 'Overall Script' } ), ( 0.0, 'Function', { 'module': 'testing', 'name': 'remote', 'dispatched': False } ) ]
  assert runner.toAssembler( [ 'testing' ] ) == { 'cookie': runner.factory_cookie, 'module': 'testing', 'function': 'remote_func', 'paramaters': 'the count "1"' }
  assert runner.status == [ ( 0.0, 'Scope', { 'description': 'Overall Script' } ), ( 0.0, 'Function', { 'module': 'testing', 'name': 'remote', 'dispatched': True } ) ]
  assert runner.variable_map == {}
  assert runner.fromAssembler( runner.factory_cookie, 'the sky is falling' ) == ( 'Accepted', 'Current State "the sky is falling"' )
  assert runner.status == [ ( 0.0, 'Scope', { 'description': 'Overall Script' } ), ( 0.0, 'Function', { 'module': 'testing', 'name': 'remote', 'dispatched': False } ) ]
  assert runner.variable_map == {}
  assert runner.run() == ''
  assert runner.done
  ...
  assert runner.toAssembler( [ 'testing' ] ) == { 'cookie': runner.factory_cookie, 'module': 'testing', 'function': 'remote_func', 'paramaters': 'the count "1"' }
  assert runner.variable_map == {}
  assert runner.fromAssembler( runner.factory_cookie, 'Bad' ) == ( 'Accepted', 'Current State "Bad"' )
  assert runner.variable_map == {}
...
# test the factory_cookie is getting rolled over correctly
