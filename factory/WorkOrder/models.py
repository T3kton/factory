...
class WorkOrder( models.Model ):
  ...
  @cinp.action( paramater_type_list=[ '_USER_', { 'type': 'Model', 'model': Drawing }, { 'type': 'String', 'choices': WORKORDER_EXECUTION_STYLE_CHOICES }, 'String', 'Map' ] )
  @staticmethod
  def new( user, drawing, execution_style, part_query=None, options=None ):
    options = options or {}

    if drawing.allow_query_in_execute and part_query:
    ...
    if part_query:
      from factor.WorkOrder.lib import get_part_list
      ...
      workorder.results = dict( [ ( i, 'new' ) for i in part_list ] )

    else:
      workorder.results[ None ] = 'new'
  ...
  def start( self ):
    ...
    from factory.WorkOrder.lib import _createJob

    if isinstance( self.results, dict ):
      for part in self.results.keys():
        _createJob( self, part )
